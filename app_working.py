...
async def retrieval(
    ...
    if not is_query_relevant(Final_Docs):
        if RAG_Type == RAGType.ADAPTIVE_RAG:
            return JSONResponse(status_code=200, content={"message": "Your query is irrelevant to the content. For more information, you may visit: https://example.com"})
        else:
            return JSONResponse(status_code=400, content={"message": "The query is irrelevant to the ingested documents."})
